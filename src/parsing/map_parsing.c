...
void check_map_components(t_container *content) {
  ...
  while (content->map[i]) {
    ...
    while (content->map[i][j]) {
      if (content->map[i][j] == 'N' || content->map[i][j] == 'E' ||
          content->map[i][j] == 'W' || content->map[i][j] == 'S') {
      ...
      if (j == 0 || i == content->map_h - 1 || i == 0 ||
          j == ft_strlen(content->map[i]) - 1) {
        if (content->map[i][j] != '1' && content->map[i][j] != ' ' &&
            content->map[i][j] != '\t')
